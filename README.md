...
| `hashc <datasource> <table> <dump-file>` | Compares the a table against the baseline dump file produced by the `hashd` command. If it finds different hash values for a row, it displays the hashes, the row number, and stops. It automatically detects the dump file range and step, if present, and acts accordingly |
